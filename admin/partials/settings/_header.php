...
$sections = array(
    'feed'     => __( 'Basic settings', 'kvs' ),
    'rules'    => __( 'Import rules', 'kvs' ),
    'post'     => __( 'Posts creation', 'kvs' ),
    'taxonomy' => __( 'Taxonomies settings', 'kvs' ),
...
<div class="kvs-setting-header">
    <h1><img src="https://www.kernel-video-sharing.com/images/logo.svg" height="60" alt="<?php _e( 'Kernel Video Sharing plugin settings', 'kvs' ); ?>" /></h1>
    ...
    <?php else: ?>
        <?php foreach( $sections as $sec=>$title ): ?>
            <a class="kvs-setting-tab<?php if( $sec === $section ) {echo ' active';} else {echo ' disabled';} ?>" aria-current="true"><?php echo $title; ?></a>
        <?php endforeach; ?>
    <?php endif; ?>
	</nav>
</div>
