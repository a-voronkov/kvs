...
class Kvs_DB {
    ...
    public function kvs_dashboard_widget(){
        ...
        if ( empty($kernel_video_sharing->reader->get_feed_url()) ) {
            ...
            echo '<p align="center">';
            echo '<button class="button button-primary button-large" onclick="document.location=\'' . $link . '\'">';
            echo __( 'Connect KVS feed to start', 'kvs' );
            echo '</button>';
            echo '</p>';
        ...
        echo '<ul><li><i class="fas fa-film"></i> <a href="edit.php?post_type=page">';
        $count = wp_count_posts( 'kvs_video' )->publish;
        printf( __( '%d videos in the library', 'my-text-domain' ), $count );
        echo '</a></li></ul>';
        ...
        if ( $cron_period === 'manual' || 
			...
			echo '<p>';
            _e( 'Automatic videos synchronization is turned off.', 'kvs' );
			echo '</p>';
            
            $link = admin_url( 'edit.php?post_type=kvs_video&page=kvs-settings' );
			echo '<p>';
            echo '<button class="button button-secondary button-small" onclick="document.location=\'' . $link . '\'">';
            ...
            if ( $last_run ) {
                ...
                echo '<p>' . sprintf( __( 'Last import was %s ago.', 'kvs' ), $time_passed ) . '</p>';
                $cron_display = '<b>' . $cron_schedules[$cron_period]['display'] . '</b>';
                echo '<p>' . sprintf( __( 'Synchronization is scheduled to run %s.', 'kvs' ), $cron_display ) . '</p>';
            } else {
                echo '<p>' . __( 'Automatic videos synchronization was not fired yet.', 'kvs' ) . '</p>';
            }
            ...
            echo '<p>' . sprintf( __( 'Next run in %s.', 'kvs' ), $next_run_r ) . '</p>';
        }
...
?><script>
    ...
    kvs_state.className = "fa fa-fw fa-circle <?php echo $color;?>";
    document.getElementById('<?php echo KVS_PREFIX;?>_dashboard_widget').children[0].children[0].appendChild(kvs_state);
</script><?php
    }
}
